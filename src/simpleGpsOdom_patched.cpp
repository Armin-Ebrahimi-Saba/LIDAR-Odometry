  ...
  void GNSSCB(const sensor_msgs::NavSatFixConstPtr &msg) {
    ...
    if (distance > 5.0) {
      ...
      if (!firstYawInit) {
        ...
        ResetOrigin(lla);
        prevYaw = yaw;
        // Recompute ENU with new origin
        ecef = gtools.LLA2ECEF(lla);
        enu = gtools.ECEF2ENU(ecef);
      }
    } else {
      if (!firstYawInit) {
      ...
      // If initialized, keep orientationReady_ true and use old yawQuat
      orientationReady_ = true;
    }
    ...
    if (firstYawInit) {
      // Rotate ENU coordinates to align with Lidar Odometry frame (X forward)
      double cy = cos(-prevYaw);
      double sy = sin(-prevYaw);
      double rx = calib_enu(0) * cy - calib_enu(1) * sy;
      double ry = calib_enu(0) * sy + calib_enu(1) * cy;
      calib_enu(0) = rx;
      calib_enu(1) = ry;
      yawQuat = tf::createQuaternionMsgFromYaw(yaw - prevYaw);
    }
    ...
    // Use the covariance set in the bag file
