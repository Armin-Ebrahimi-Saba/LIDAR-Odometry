...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void updateInitialGuess() {
    ...
    if (cloudKeyPoses3D->points.empty()) {
      ...
      // We ALWAYS initialize using IMU/Lidar first, even if useGPS is true.
      // The GPS factors will simply be added later in addGPSFactor() when they become available!
      ...
      ROS_INFO("System initialized with IMU/Lidar (GPS will be added when available)");
      return;
    }
