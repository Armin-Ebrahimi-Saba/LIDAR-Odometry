...
| Aspect | With GNSS | Without GNSS |
| :--- | :--- | :--- |
| **Focus** | GNSS data dominates | LiDAR points |
| **Strengths** | Robust against fundamental errors in map creation | Very good 3D map |
| **Weaknesses** | Produced 3D map is of lower quality | IMU drift and incorrect association of geometries by ICP |

Furthermore, the following observations were made during the evaluation:
- It was possible to successfully implement LIO-SAM 6Axis and approximate the entire path of Test 1.
- LIO-SAM was evaluated twice: once with the integration of raw GNSS sensor data, and once without.
- As can be seen in diagram XX, the path generated by LIO-SAM using GNSS data is more precise than the path without GNSS.
...
When analyzing these results, it becomes apparent (as also demonstrated in video XX) that at time XXX in the rosbag data, a rapid rotation occurs that cannot be adequately compensated for by the low-cost IMU. Consequently, LIO-SAM is unable to recover from the resulting confusion in the point clouds.
