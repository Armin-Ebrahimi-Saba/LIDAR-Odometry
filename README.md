...
5. **Start the SLAM Node**: Start the container using Docker Compose. By default, it runs without GNSS data:
   ...
   **On Linux/macOS:**
   ```bash
   USE_GNSS=true docker-compose up --build
   ```
   **On Windows (PowerShell):**
   ```powershell
   $env:USE_GNSS="true"; docker-compose up --build
   ```
...
The 6-axis LIO-SAM framework was successfully implemented and evaluated across the entire trajectory of Test 1. We compared the system's performance under two distinct configurations: with and without the integration of raw GNSS data provided by the Ouster sensor.

> The final evaluation was conducted using custom Python scripts running outside of ROS, utilizing standard libraries to compare the drifting trajectories against the ground truth GNSS data. *(Note: Raw data and outputs have been omitted from this streamlined repository delivery. All plotting scripts are available in the corresponding folder).*

### 3.1 Summary

| Aspect | LIO-SAM with GNSS | LIO-SAM without GNSS |
| :--- | :--- | :--- |
| **Focus** | GNSS data prioritization | LiDAR feature prioritization |
| **Strengths** | Prevents severe map distortion and drifting | High-fidelity 3D mapping |
| **Weaknesses** | Reduced point cloud density and visual fidelity | Susceptible to IMU drift and perceptual aliasing |

### 3.2 Path and Deviation (RMSE)
...
2. **Failure during Fast Rotation (Perceptual Aliasing):** LIO-SAM relies on feature-based scan matching (a variant of the Iterative Closest Point or ICP algorithm), extracting and matching geometric features like edges and planar surfaces between consecutive LiDAR scans to estimate odometry. During the rapid rotation around the robot's vertical axis, the spatial overlap between consecutive scans is drastically reduced. In this scenario, the algorithm likely encountered similar-looking edges in the environment (perceptual aliasing) and mismatched them. This faulty data association forces an incorrect heading estimation, causing the mapped trajectory to diverge and "curve" away.
3. **IMU Pre-integration Limits:** During rapid rotations, low-cost IMUs accumulate significant noise, causing their pre-integration to drift. LIO-SAM relies heavily on the IMU to de-skew LiDAR sweeps and to provide an initial guess for the scan-matching algorithm. If the IMU provides a faulty initial rotation guess due to high noise or error tolerance limits, the ICP algorithm converges into a wrong local minimum, matching incorrect geometric features.
4. **Motion Skew/Blur:** A LiDAR sensor requires time to complete a 360-degree sweep (typically 100 ms). Fast rotation during this sweep distorts the point cloud, causing motion skew. If the IMU data is noisy, the de-skewing process fails, leaving the scan warped. These warped point clouds cannot be accurately matched against the existing map, further compounding the registration error.
5. **GNSS as a Global Constraint:** GNSS data provides an absolute global reference that corrects local heading and translation errors. Without it, the system lacks a mechanism to correct a false heading induced by a bad scan match during a rapid rotation, resulting in permanent map distortion.

### 3.5 Conclusion

Overall, LIO-SAM demonstrated strong performance, though the results highlight a clear trade-off between global trajectory accuracy and local map fidelity. The optimal configuration ultimately depends on the specific use case: incorporating GNSS data is highly favorable when robust localization and accurate global orientation are the priorities. Conversely, if preserving high-resolution 3D mapping details is the primary goal, running LIO-SAM without GNSS yields vastly superior visual results, albeit at the risk of trajectory drift during rapid rotations. 

To mitigate these limitations in the future, upgrading to a higher-grade IMU could significantly reduce drift and prevent severe misalignments. Furthermore, an optimal balance of accurate path tracking and crisp map generation could likely be achieved by fine-tuning the GNSS trust weights, allowing LIO-SAM to benefit from global constraints without severely distorting the local point cloud.
