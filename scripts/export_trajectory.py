...
Usage:
  python3 scripts/export_trajectory.py <traj_lidar.txt> <gnss_csv> [output.csv] [--offset OFFSET]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Export aligned GLIM trajectory (Lat/Lon/Alt + NED velocity) to CSV.")
    ...
    parser.add_argument("out_path", nargs="?", default="trajectory_latlon_ned.csv", help="Output CSV path")
    ...
    # Apply alignment to the FULL GLIM trajectory
    ...
    print(f"Wrote {len(t_est)} poses to {args.out_path}")
