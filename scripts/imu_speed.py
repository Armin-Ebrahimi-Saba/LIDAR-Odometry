...
./data/px4_msgs, fetched by the pipeline's fetch stage).

Usage:
    python scripts/imu_speed.py 1                 # speed at LiDAR frame 1
    python scripts/imu_speed.py 1500 --source odometry
    python scripts/imu_speed.py 0 --config /configs/config_test1.yaml --px4-msgs-dir ./data/px4_msgs
"""
import argparse
...
_SOURCES = {
    "local_position": (
        ...
        "data/px4_msgs/msg/VehicleLocalPosition",
        lambda m: (m.vx, m.vy, m.vz),
    ),
    "odometry": (
        "/fmu/out/vehicle_odometry",
        "data/px4_msgs/msg/VehicleOdometry",
...
def _typestore(px4_msgs_dir: Path, type_name: str):
    ...
    if not msg_file.exists():
        raise SystemExit(
            f"Cannot find {msg_file}. Run the pipeline's fetch stage (or clone px4_msgs release/1.17), or pass "
...
def imu_speed(config_path: str, frame: int, source: str = "local_position",
              px4_msgs_dir: str = "./data/px4_msgs", lidar_topic: str | None = None):
...
def main():
    ...
    ap.add_argument("--px4-msgs-dir", default="./data/px4_msgs",
                    help="path to the px4_msgs checkout (default: ./data/px4_msgs)")
