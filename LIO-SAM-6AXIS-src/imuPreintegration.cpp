...
class TransformFusion : public ParamServer {
 ...
 public:
  ...
  TransformFusion() {
    ...
    subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("lio_sam_6axis/mapping/odometry",
                                                        5,
 ...
 public:
  ...
  IMUPreintegration() {
    subImu = nh.subscribe<sensor_msgs::Imu>(imuTopic,
                                            ...
                                            this,
                                            ros::TransportHints().tcpNoDelay());
    subOdometry = nh.subscribe<nav_msgs::Odometry>("lio_sam_6axis/mapping/odometry_incremental",
                                                   5,
    ...
    correctionNoise = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6)
        << 0.05, 0.05, 0.05, 0.1, 0.1, 0.1).finished()); // rad,rad,rad,m, m, m
    correctionNoise2 =
        gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1, 1, 1, 1, 1, 1).finished()); // rad,rad,rad,m, m, m
...
//      p->n_gravity = gtsam::Vector3(imuGravity_N[0], imuGravity_N[1], imuGravity_N[2]);
      noiseModelBetweenBias =
          (gtsam::Vector(6)
              << imuAccBias_N[0], imuAccBias_N[1], imuAccBias_N[2], imuGyrBias_N[0], imuGyrBias_N[1], imuGyrBias_N[2]).finished();
    } else {
      noiseModelBetweenBias =
          (gtsam::Vector(6) << imuAccBiasN, imuAccBiasN, imuAccBiasN, imuGyrBiasN, imuGyrBiasN, imuGyrBiasN).finished();
    }
    ...
    imuIntegratorOpt_ =
        new gtsam::PreintegratedImuMeasurements(p, prior_imu_bias); // setting up the IMU integration for optimization
  }
